...
@open_id.after_login
async def after_login(resp):
    try:
        root = Macaroon.deserialize(quart.session.pop("macaroon_root"))
    except KeyError:
        return (
    ...
    quart.session["authentication_token"] = binary_serialize_macaroons(
        [root, bound]
    ).decode("utf-8")

    if not resp.nickname:
        return quart.redirect(login_url)
...
def logout():
    return_to = quart.request.args.get("return_to") or quart.request.path

    # Protect against redirect loop if return_to is logout
    if return_to == "/logout":
        return_to = "/"

    empty_session(quart.session)

    return quart.redirect(return_to)
