...
from quart import request, session, redirect, current_app, url_for
...
class OpenID(object):
    ...
    async def get_next_url(self):
        ...
        """
        self.next_url = (
            self.check_safe_root((await request.values).get("next"))
    ...
    async def get_current_url(self):
        ...
        r = request.base_url + "?next=" + url_quote(await self.get_next_url())
        return r
    ...
    async def get_success_url(self):
        ...
        :internal:
        """
        return (await self.get_current_url()) + "&openid_complete=yes"
    ...
    def loginhandler(self, f):
        ...
        async def decorated(*args, **kwargs):
            ...
            args.update((await request.form).to_dict())
            openid_response = consumer.complete(
                args, await self.get_current_url()
            )
            if openid_response.status == SUCCESS:
                return await self.after_login_func(
            ...
            return redirect(await self.get_current_url())
    ...
    async def try_login(
        ...
        return redirect(
            auth_request.redirectURL(
                trust_root, await self.get_success_url(), immediate=immediate
            )
        )
